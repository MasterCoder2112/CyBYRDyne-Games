...
//Makes sure user doesn't stay authenticated after logging out
let authTest = false;
...
//Authenticate the facebook account, and force user to reauthenticate each log in attempt
...
//Logout extension which logs the user out of the session
...
/*
 * When user is successfully logged in, set authTest to true
 * to show the user is truly logged in. When the user logs out
 * of the session, for some reason the user is still 'authenticated'
 * in terms of passport, so authTest allows it so that in case it
 * is a different user it will not allow him/her to access the site
 * based on another users authentication.
 */
